...
void	ft_raycast(t_game *g)
{
	...
	while (++x < WIN_WIDTH)
	{
		...
		g->calc.y.start = -g->calc.line_height / 2 + WIN_HEIGHT / 2;
		if (g->calc.y.start < 0)
			g->calc.y.start = 0;
		g->calc.y.end = g->calc.line_height / 2 + WIN_HEIGHT / 2;
		if (g->calc.y.end >= WIN_HEIGHT)
			g->calc.y.end = WIN_HEIGHT - 1;
		...
		for (int y = g->calc.y.start; y < g->calc.y.end; y++)
		{
